...
public class IonItemTests : IonTestContext
{
    // ---------------------------------------------------------------------------
    // Render tests
    // ---------------------------------------------------------------------------
    ...
    [Fact]
    public async Task IonItemRendersCorrectly()
    {
        var cut = Render<IonItem>();
    ...
    public async Task WithMode_RendersCorrectly(string mode)
    {
        ...
        var cut = Render<IonItem>(parameters => parameters
            .Add(p => p.Mode, mode));
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task WithDetail_RendersCorrectly(bool value)
    {
        VerifySettings settings = new();
        settings.UseTextForParameters($"value={value}");

        var cut = Render<IonItem>(parameters => parameters
            .Add(p => p.Detail, value));
    ...
    [Theory]
    [InlineData(IonItemLines.Full)]
    [InlineData(IonItemLines.Inset)]
    [InlineData(IonItemLines.None)]
    ...
    {
        var cut = Render<IonItem>(parameters => parameters
            .AddChildContent("<ion-label>Item Label</ion-label>"));
