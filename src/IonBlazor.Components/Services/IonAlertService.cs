...
/// <summary>
/// Scoped DI service for programmatically presenting an Ionic alert.
/// Register via <c>services.AddIonBlazor()</c> and inject with <c>@inject IonAlertService</c>.
/// </summary>
public sealed class IonAlertService(IJSRuntime jsRuntime)
{
    /// <summary>
    /// Presents an alert configured by the supplied <see cref="AlertControllerOptions"/> action.
    /// </summary>
    public async ValueTask PresentAsync(Action<AlertControllerOptions> configure)
    {
        ...
        await using IJSObjectReference jsComponent = await jsRuntime.ImportAsync(nameof(IonAlertService));
